...
#include "control/defender_half_gate.h"

namespace { static DefenderHalfGate kDefGate(0.05); }
...
BT::NodeStatus robotTrackPelvis::tick()
{
    ...
    if ((abs(_interface->ballYawToPelvis)<=0.20)
        && (abs(_interface->ballPositionInPelvis(0)) <= 0.55)
        && (abs(_interface->ballPositionInPelvis(1)) <= 0.3))
    {
        _interface->locoClient.Move(0, 0, 0);
    }
    else
    {
        ...
        // === 防守半场硬闸 ===
        double rx = _interface->homoMatPelvisToField(0,2); // 机器人在场地系的 x
        Twist cmd{vx, vy, vyaw};
        kDefGate.clipVelocity(rx, cmd);
        _interface->locoClient.Move(cmd.vx, cmd.vy, cmd.wz);
        // ====================
    }
    return BT::NodeStatus::SUCCESS;
}
...
BT::NodeStatus robotTrackField::tick()
{
    ...
    double vyaw_adjust = saturation(vtheta, Vec2<double>(-0.4,0.4));
    ...
    // === 防守半场硬闸 ===
    double rx = _interface->homoMatPelvisToField(0,2);
    Twist cmd{vx, vy, vyaw};
    kDefGate.clipVelocity(rx, cmd);
    _interface->locoClient.Move(cmd.vx, cmd.vy, cmd.wz);
    // ====================
...
BT::NodeStatus kick::tick()
{
    ...
    // === 向对方半场正中央 (2.25, 0) 推球（在“防守半场硬闸”之前计算） ===
    constexpr double target_x = 2.25;
    constexpr double target_y = 0.0;

    const double bx = _interface->ballPositionInField(0);
    const double by = _interface->ballPositionInField(1);

    // 方向（场地系）：从球指向目标点
    double dirx_f = target_x - bx;
    double diry_f = target_y - by;
    double dist   = std::hypot(dirx_f, diry_f);

    // 到达端线或非常接近目标 → 停
    if (_interface->ballPositionInField(0) >= 4.5 || dist < 1e-6) {
    ...
    const double theta = _interface->locateResult->msg_.robot2field_theta(); // 机体→场地的偏航
    const double c = std::cos(theta), s = std::sin(theta);
    // v_body = R(-theta) * v_field
    double dirx_b =  c * dirx_f + s * diry_f;
    double diry_b = -s * dirx_f + c * diry_f;

    // 推进速度（机体系）
    const double p = 0.6;
    vx = saturation(p * dirx_b, Vec2<double>(-0.6, 0.6));
    vy = saturation(p * diry_b, Vec2<double>(-0.6, 0.6));

    // 朝向误差（让机体大致朝向“球→目标”的方向），可选
    double heading_err = std::atan2(diry_f, dirx_f) - theta;  // wrap 到 [-pi, pi]
    if (heading_err > M_PI)  heading_err -= 2*M_PI;
    if (heading_err < -M_PI) heading_err += 2*M_PI;
    vyaw = saturation(0.6 * heading_err, Vec2<double>(-0.6, 0.6));
    }
