...
void Interface::lowStateHandle()
{
    ...
    // 机器人越界
    robot_out = (std::fabs(rx) > FIELD_X) || (std::fabs(ry) > FIELD_Y);

    // 球越界（只在看见球时更新，避免无球时反复抖动）
    if (ballDetected) {
        ball_out = (std::fabs(ballPositionInField(0)) > FIELD_X) ||
                   (std::fabs(ballPositionInField(1)) > FIELD_Y);
    }

    // 触发“硬停”：一旦球或机器人越界 → 置位 halted；复用 pause_signal
    if ((ball_out || robot_out) && !halted) {
        halted = true;
        pause_signal = true;          // 上层若有检查会自动停
        locoClient.StopMove();        // 立停（等同 Move(0,0,0)）
        // 如需让云台断使能（可选），取消下面注释：
        // servoCmd->msg_.cmds()[0].mode() = 0;
        // servoCmd->msg_.cmds()[1].mode() = 0;
        // servoCmd->unlockAndPublish();
    }

    // 处于 halted 期间：每帧强制零速，覆盖上层任何速度指令
    if (halted) {
        locoClient.StopMove();
    }
