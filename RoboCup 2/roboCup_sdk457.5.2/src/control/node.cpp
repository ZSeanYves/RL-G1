...
{
    // === 目标：对方半场正中央 (2.25, 0) ===
    constexpr double kFieldW = 9.0;                 // 比赛用地宽 9m
    const double target_x = +kFieldW * 0.25;        // 2.25
    ...
    // 计算从球指向目标点的（场地系）方向
    double dirx = target_x - bx;
    double diry = target_y - by;
    const double n = std::hypot(dirx, diry);
    if (n < 1e-6) {
        _interface->locoClient.Move(0, 0, 0);
        return BT::NodeStatus::SUCCESS;
    }
